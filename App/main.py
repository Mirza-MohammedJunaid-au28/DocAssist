...
def doctor_timing():
    ...
    specialist, doctor_name, doctor_time, doctor_day = [row[1] for row in rows], [row[2] for row in rows], [row[3] for row in rows], [row[4] for row in rows]
...
def available_doctors():
    ...
    result = fetchAll_Query("SELECT specialist,name,timing FROM doctor_timing WHERE day = %s;",(current_day,))
    return result

def pending_appointment():
    pending_count = fetchOne_Query("SELECT COUNT(*) FROM appointments WHERE DATE(doctor_date) = CURRENT_DATE;")[0]
    completed_count = fetchOne_Query("SELECT COUNT(*) FROM appointment_history WHERE LEFT(\"Appointment Time\", 10)::date = CURRENT_DATE;")[0]
    return pending_count,completed_count

def doctor_and_patient():
    result1 = fetchOne_Query("SELECT COUNT(*) FROM doctor_timing;")[0]
    result2 = fetchOne_Query("SELECT COUNT(*) FROM user_data where type = 'user';")[0]
...
def top_doctor():
    ...
    for i in range(max_iterations):
        result2 = fetchOne_Query("SELECT profile_photo FROM user_demographic where name = %s;",(result[i][0],)) [0]
...
def main():
    try:
        if request.method == 'GET':
            ...
            result = fetchOne_Query("SELECT * FROM user_data WHERE gmail = %s;",(gmail,))
            
            if result and password == result[2]:
                res = fetchOne_Query("SELECT * FROM user_demographic WHERE user_id = %s;",(result[0],))
    ...
    try:
        if request.method == 'GET':
            ...
            if session and session['access'] :
                ...
                password = request.form.get('password')
                last_row_data = fetchOne_Query("SELECT * FROM user_data ORDER BY user_id DESC LIMIT 1;")
...
def home():
    try:
        ...
        if session['access'] == 'user':
            ...
            res1 = fetchAll_Query("SELECT * FROM appointments WHERE user_id = %s;", (session['user_id'],))
            res2 = fetchOne_Query("SELECT * FROM user_demographic WHERE user_id = %s;", (session['user_id'],))

            img = res2[6] if res2 else None
            base64_image = base64.b64encode(io.BytesIO(img).getvalue()).decode('utf-8') if img else None
...
def demographics():
    try:
        if request.method == 'GET':
            if not session:
                data_received = request.args.get('data', 'default_value_if_not_provided')
                temp_email = request.args.get('email', 'default_value_if_not_provided')
                temp_password = request.args.get('password', 'default_value_if_not_provided')
            ...
            if not session:
                ...
                insert_Query("INSERT INTO user_data (user_id,gmail, password, type) VALUES (%s,%s, %s, %s);",(user_id,email, password, 'user'))


                insert_Query("""INSERT INTO user_demographic (user_id, name, gender, contact, blood_group, dob, profile_photo, address) VALUES (%s, %s, %s, %s, %s, %s, %s, %s);""",(user_id,name,gender,contact,blood_group,dob,psycopg2.Binary(photo_data),address))
...
def dashboard():
    if session['access'] == 'doctor':
        res = fetchAll_Query("SELECT * FROM appointments WHERE doctor_name = %s;",(session['name'],))
        user_data = []
        for row in res:
            userr = fetchOne_Query("SELECT * FROM user_demographic WHERE user_id = %s;",(row[1],))
            userr = (row[0],) + userr
...
def recording():
    if session['access'] == 'doctor':
        ...
        if audio_data:
            ...
            # transcription = pipe(filepath)["text"]
            end_time = time.time()
            print('Time Taken to Transcript : ', end_time - start_time)
            print('Transcription:',transcription)

            # sentiment = max(emotion(filepath), key=lambda x: x['score'])['label']
            sentiment = classifier(transcription)[0]['label']

            output1 = pratham(transcription)
            output2 = gladys(transcription)

            res = fetchOne_Query("SELECT * FROM appointments WHERE srno = %s;",(appointment_id,))
            doctor_name = res[3]

            regular_medicines = list_to_string(output2['regular_medicines'])
            last_prescribed_medicines = list_to_string(output2['last_prescribed_medicines'])
            current_prescribed_medicines = list_to_string(output2['current_prescribed_medicines'])
            immunization_history = list_to_string(output2['immunization_history'])
            ...
            data = (appointment_id,user_id,doctor_name,current_datetime,regular_medicines,last_prescribed_medicines,current_prescribed_medicines,immunization_history,detected_symptoms,detected_diseases,detected_surgery,detected_tests,conversation_tone,voice_conversation)
...
def profile():
    if request.method == 'GET':
        select_query = sql.SQL("SELECT * FROM user_demographic WHERE user_id = %s;")
        ...
        if res:
            ...
            base64_image = base64.b64encode(image_bytes.getvalue()).decode('utf-8')
            res = tuple(value if i != 6 else base64_image for i, value in enumerate(res))
...
def lastAppointment():
    if request.method == 'GET':
        ...
        if session['access'] == 'user':
            column_names = ["Appointment ID", "Doctor Name", "Appointment Time", "Current Prescribed Medicine", "Detected Symptoms", "Detected Diseases", "Detected Surgery", "Detected Tests"]
            ...
            column_names = ["Appointment ID", "Patient ID", "Appointment Time", "Current Prescribed Medicine", "Detected Symptoms", "Detected Diseases", "Detected Surgery", "Detected Tests"]
            select_query1 = sql.SQL("SELECT ({}) FROM appointment_history WHERE {} = %s;").format(
            sql.SQL(', ').join(map(sql.Identifier, column_names)),
        ...
        select_query2 = sql.SQL("SELECT * FROM user_demographic WHERE user_id = %s;")
...
def admin():
    if request.method == 'GET':
        ...
        select_query = sql.SQL("SELECT name,profile_photo FROM user_demographic WHERE user_id = %s;")
        cursor.execute(select_query, (session['user_id'],))
        res = cursor.fetchone()
        img = res[1] 
        image_bytes = io.BytesIO(img)
